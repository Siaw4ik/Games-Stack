...
export function game1() {
  ...
  main.innerHTML = "";
  const page = `<div class="main__container _container">
  <h1 class="main__title">Conversion types-game</h1>
  <div class="main__settings">
    <div class="main__timer timer">
      <div class="timer__text">Time:</div>
      <div id="minute" class="timer__minutes"></div>
      <div id="second" class="timer__seconds"></div>
    </div>
    <div class="main__buttons">
      <button id="startBtn" class="main__start-button button">Start</button>
      <button id="resetBtn" class="main__reset-button button" disabled="true">
      Start again
      </button>
    </div>
    <div id="answers-count" class="main__answers-count">Correct answers: 0/15 </div>
  </div>
  <h2 id="start-message" class="main__start-message">To play the game press Start</h2>
  <div id="main-game" class="main__body">
    <div id="question" class="main__question"></div>
    <div class="main__answers-block">
      <div class="main__answer"></div>
      <div class="main__answer"></div>
      <div class="main__answer"></div>
      <div class="main__answer"></div>
    </div>
    <button id="nextBtn" class="main__next-button button" disabled="true">
      Next
    </button>
    <div id="complete" class="main__complete">
      <div id="message" class="main__complete-message">
        <div id="message-text" class="main__complete-text"></div>
        <div id="message-close" class="main__complete-close">
...
let questionsForGame = questions.slice(0, 15);
let answersCount = 0;
...
function viewFinishMessage() {
  ...
  messageText.innerText = `You are completed 15 answers 
  with ${answersCount} correct answers 
  in ${minute.innerHTML.slice(0, minute.innerHTML.length - 1)} minute(s) ${
    second.innerHTML
  } second(s)`;
  const completeContainer = document.getElementById("complete") as HTMLElement;
  completeContainer.classList.add("open");

  function closeCompleteMessage(): void {
    if (completeContainer.classList.contains("open")) {
      document.body.addEventListener("click", (e) => {
        if (
          (<HTMLButtonElement>e.target).closest("#message-close") ||
          !(<HTMLButtonElement>e.target).closest("#message")
        ) {
...
export const startGame = (): void => {
  ...
  const mainGame = document.getElementById("main-game") as HTMLElement;
  ...
  document.body.addEventListener("click", (e) => {
    if ((<HTMLButtonElement>e.target).id === "startBtn") {
...
export const clickAnswer = (): void => {
  const answersAll = Array.from(document.querySelectorAll(".main__answer"));
  const question = document.getElementById("question") as HTMLElement;
  ...
  document.body.addEventListener("click", (e) => {
    ...
    if ((<HTMLDivElement>e.target).className === "main__answer") {
      clearInterval(timer);
      if (
        answer.innerHTML === correct &&
        answersAll.filter((el) => el.classList.contains("correct")).length ===
          0 &&
        answersAll.filter((el) => el.classList.contains("incorrect")).length ===
          0
      ) {
        answer.classList.add("correct");
        answersCount += 1;
        correctAnswers.innerHTML = `Correct answers: ${answersCount}/15`;
      ...
      if (
        ...
        answersAll.filter((el) => el.classList.contains("correct")).length > 0
      ) {
...
export const clickNext = (): void => {
  ...
  const nextBtn = document.getElementById("nextBtn") as HTMLButtonElement;
  document.body.addEventListener("click", (e) => {
    if ((<HTMLButtonElement>e.target).id === "nextBtn") {
...
export const startAgain = (): void => {
  document.body.addEventListener("click", (e) => {
    ...
    if ((<HTMLButtonElement>e.target).id === "resetBtn") {
      questions.sort(
        (a, b) => a.id - a.id + Math.random() - (b.id - b.id + Math.random())
      );
